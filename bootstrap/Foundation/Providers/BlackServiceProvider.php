...
abstract class BlackServiceProvider
{
    public $app;
    ...
    final public static function setup(&$app, array $providers)
    {
        ...
        collect($providers)
            ...
            ->each(fn (BlackServiceProvider $provider) => $run_when_exists($provider, 'register'))
            ->each(fn (BlackServiceProvider $provider) => $run_when_exists($provider, 'boot'));
    }
}
